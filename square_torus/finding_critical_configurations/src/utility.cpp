...
// aquare with opposite edges identified and edge length 1.
//----------------------------------------
double _periodic_x::operator()(double x, double y, usize s) {
  r[0] = x;
  r[1] = y;
  d = p * r;
...
double _periodic_y::operator()(double x, double y, usize s) {
  r[0] = x;
  r[1] = y;
  d = p * r;
...
//----------------------------------------
arma::vec jacob_shift(double w, double r, const arma::vec & coords,
                      _periodic_x & periodic_x, _periodic_y & periodic_y) 
{
  arma::vec output(jacobian(w, r, coords, periodic_x, periodic_y));
