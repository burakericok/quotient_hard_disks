...
% Two disks
% x0 = [0., 0.577, 0.5, 0.289];
% x0 = [-0.25, 0., 0.25, 0.];

% Three disks
% x0 = [0., 0.577, 0.5, 0.289, 0.,0.];
% x0 = [0.5, 0., 0.25, 0.433, 0.25, -0.433];
% x0 = [-0.25, 0., 0.25, 0., 0., 0.433]; % REPEAT OF ONE ABOVE
% x0 = [0.358, -0.323, -0.475, -0.034, -0.309, 0.254]; % MISTAKE IN COORDS
% x0 = [-0.33, 0., 0., 0., 0.33, 0.];

% Four disks
% x0 = [0.5, 0., 0.25, 0.433, 0.25, -0.433, 0., 0.];
% x0 = [0.358, -0.323, 0.475, -0.034, -0.309, 0.254, 0.282, 0.402]; % MISTAKE IN COORDS
% x0 = [-0.33, 0., 0., 0., 0.33, 0., 0., 0.35];
% x0 = [0.315, 0.170, -0.0589, 0.388, 0.0654, -0.263, -0.310, -0.0470];
% x0 = [-0.375, 0., -0.125, 0., 0.125, 0., 0.375, 0.];
% x0 = [0.845, 0.194, 0.095, 0.627, 0.19, 0.283, 0.499, 0.105];
% x0 = [0.572, 0.908, 0.0072, 0.908, 1.0, 0.472, 0.572, 0.472];
% x0 = [0.143, 0.604, 0.326, 0.921, 0.46, 0.0554, 0.143, 0.238];
% x0 = [0.979, 0.612, 0.946, 0.179, -0.512, 0.179, 0.479, 0.612];
...
for a = 1:n_disk
    xa = periodic_x(x0(2 * a - 1), x0(2 * a), 0);
    xb = periodic_y(x0(2 * a - 1), x0(2 * a), 0);
    x0(2 * a - 1) = xa;
    x0(2 * a) = xb;
end
...
for a = 1:n_disk
    for b = (a + 1):n_disk
        filter = (x(ind:(ind + 4)) > eps) & (x(ind:(ind + 4)) < 1.005 * r0);
...
% draw fundamental cell and some of its copies for visualization
% figure(1)
% hold on
...
% rectangle( 'position', [ -0.5 -1.5 1 1 ] )
% rectangle( 'position', [ 0.5 0.5 1 1 ] )
% rectangle( 'position', [ 0.5 -1.5 1 1 ] )
% rectangle( 'position', [ -1.5 0.5 1 1 ] )
% rectangle( 'position', [ -1.5 -1.5 1 1 ] )
% hold off
