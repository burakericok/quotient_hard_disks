...
function plotting(vars) 
    ...
    for c = 1:nDisk
        x = periodic_x(vars(2 * c - 1), vars(2 * c),0);
        y = periodic_y(vars(2 * c - 1), vars(2 * c),0);
        for a = 1:size(shift , 1)
            lifted_disks(a, :, c) =  [x , y] + shift(a, 1) * [1., 0.]  + shift(a, 2) * [0., 1.0];
        end
    end

    for a = 1:nDisk
        x = periodic_x(vars(2 * a - 1), vars(2 * a),0);
        y = periodic_y(vars(2 * a - 1), vars(2 * a),0);
