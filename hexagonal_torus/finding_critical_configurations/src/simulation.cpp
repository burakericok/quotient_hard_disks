...
#pragma omp parallel
    {
      ...
      bool inv_quad_step;
      arma::vec x0, x1, g0, g1, dir, ndir, v;
      ...
      arma::uvec contacts, colors, perm, boolvec;
...
#pragma omp for schedule(dynamic)
      for (usize search = 0; search < RunConfig.n_trials; ++search) {
        for (a = 0; a < x0.n_elem; a += 2) {
          x0[a] = rng();
          x0[a + 1] = rng() * SQRT3_DIV2;
        }

        for (a = 0; a < RunConfig.w_increments + 1; ++a) {
          // Conjugate gradient method to minimize residual.
          w  = 200. * pow(1.2, a);
          r0 = arma::min(radius(x0, periodic_xy));
          E0 = energy(w, r0, x0, periodic_xy);
          g0 = jacob_shift(w, r0, x0, periodic_xy);
          ...
          for (b = 0; b < MAX_CG_ITER; ++b) {
            ...
            xb = SQRT_EPS;
            fb = energy(w, r0, x0 + xb * ndir, periodic_xy);
            while (fb > fa && xb > EPS) {
            ...
            if (q > EPS) {
              ...
              if (xc > (MAX_BND_STEP + 1.) * xb) {
                ...
                fc = energy(w, r0, x0 + xc * ndir, periodic_xy);
            ...
            while (fc < fb) {
              ...
              if (t > EPS) {
                ...
                if (xd > xc + MAX_BND_STEP * p) {
                  ...
                  fd = energy(w, r0, x0 + xd * ndir, periodic_xy);
            ...
            for (c = 0; c < MAX_LS_ITER; ++c) {
              ...
              if (fabs(xb - m) > tol2 - 0.5 * (xc - xa)) {
                ...
                fu = energy(w, r0, x0 + xu * ndir, periodic_xy);
            ...
            g1 = jacob_shift(w, r0, x1, periodic_xy);
        ...
        // Postprocessing
        for (size_t i = 0; i < RunConfig.n_disks; i++) { x0.subvec(2*i,2*i+1) = periodic_xy(x0[2*i],x0[2*i+1],0); }

        // construct adjacency matrix
        pair_dist = radius(x0,periodic_xy);
        ...
        hess_eigs = arma::eig_sym(hessian(w, r0, x0, periodic_xy));
