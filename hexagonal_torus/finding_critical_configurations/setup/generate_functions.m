...
syms periodic_x0(pos) periodic_x1(pos) periodic_x2(pos)...
     periodic_x3(pos) periodic_x4(pos) periodic_x5(pos)...
     periodic_x6(pos);
syms periodic_y0(pos) periodic_y1(pos) periodic_y2(pos)...
     periodic_y3(pos) periodic_y4(pos) periodic_y5(pos)...
     periodic_y6(pos);
syms w r;

R = sym('R', [1, 7 * n_disks * (n_disks - 1) / 2]);
inc = 0;
% % self interactions
% for a = 1:n_disks
%     inc = inc + 1;
...
%     inc = inc + 1;
%     R(inc) = sqrt(subs(periodic_x5, pos, 0.)^2 + subs(periodic_y5, pos, 0.)^2) / 2.;
%     inc = inc + 1;
%     R(inc) = sqrt(subs(periodic_x6, pos, 0.)^2 + subs(periodic_y6, pos, 0.)^2) / 2.;
% end
% other interactions
for a = 1:n_disks
    for b = (a + 1):n_disks
        ...
        inc = inc + 1;
        R(inc) = sqrt(subs(periodic_x5, pos, x(a) - x(b))^2 + subs(periodic_y5, pos, y(a) - y(b))^2) / 2.;
        inc = inc + 1;
        R(inc) = sqrt(subs(periodic_x6, pos, x(a) - x(b))^2 + subs(periodic_y6, pos, y(a) - y(b))^2) / 2.;
    end
end
