...
\begin{appendices}
	...
	Finally, the descriptors $\hat{z}_{\vec{k}}$ in Eq.\ \ref{eq:z_hat_k} are not all independent. Let $\vec{x}$ and $\vec{a}$ respectively be the coordinates of a point in the $xy$-coordinate system and $a_1a_2$-coordinate system as shown in Fig.\ \ref{fig:app2}. \red{}
	
	Let $\mat{T}$ be the forward transformation matrix such that $\vec{a} = \mat{T} \vec{x}$ and $\vec{x} = \mat{T}^{-1} \vec{a}$. 
