...
\bmhead{Data and code availability} The datasets and source codes associated with this study are available in the "quotient\_hard\_disks" repository, \url{https://github.com/burakericok/quotient_hard_disks}.
...
\begin{appendices}
	...
	\red{
	This section provides proofs and supporting materials to show the relations among the descriptors. The procedure to calculate the descriptors $z_{\vec{k}}$ is illustrated in Fig.\ \ref{fig:app1}. For a configuration $\config{p}$ whose coordinates are given in $xy$-coordinate system, the first step is to find the corresponding coordinates of the disks in the $a_1a_2$-coordinate system, and define the function $f(\vec{a})$ using Eq.\ \ref{eq:f}. Observe that the resulting function will be the same for each $n!$ permutations of the disk labels. Then, the complex coefficients of the infinite expansion in Eq.\ \ref{eq:f} are calculated using the orthogonality of complex exponentials as shown in Eq.\ \ref{eq:c_k}. The final step is then to take the modulus of these complex coefficients using Eq.\ \ref{eq:z_k}. Observe that the complex coefficients and the descriptors are sorted with respect to the distance of $\vec{k}$ to the origin. Moreover, observe that the complex coefficients $c_{\vec{k}}$ and $c_{-\vec{k}}$ have the same descriptors $z_{\vec{k}}$ for a given configuration.}
	
	\begin{figure}
		\centering
		\includegraphics[width=1.0\columnwidth]{app1.eps}
		\caption{\red{$3!$ permutations of a given configuration will result in the same function $f(\vec{a})$ after using Eq.\ \ref{eq:f}. The complex coefficients are calculated using Eq.\ \ref{eq:c_k}. Finally, the descriptors $z_{\vec{k}}$ are then defined as the modulus of the complex coefficients.}}
		\label{fig:app1}
	\end{figure}


	\red{The complex coefficients $c_{\vec{k}}$ is equal to $c^{\ast}_{-\vec{k}}$ where $^{\ast}$ denotes complex conjugate.}
	\begin{proof}
		\red{Let $\config{p}$ be a configuration and $\vec{p}$ be the coordinates of the disks in $a_1a_2$-coordinate system.
		\begin{align*} 
	...
	\red{Let $\config{p}$ be a configuration and $\vec{p}$ be the coordinates of the disks in $a_1a_2$-coordinate system.}
	\begin{proof}
		\red{Using the fact that $c_{\vec{k}} = c^{\ast}_{-\vec{k}}$
		\begin{align*} 
	...
	Finally, the descriptors $\hat{z}_{\vec{k}}$ in Eq.\ \ref{eq:z_hat_k} are not all independent. \red{Figure \ref{fig:app2} shows two different coordinate systems ($xy$- and $a_1a_2$-) associated with the hexagonal domain. The reason why there are two different coordinate systems is that representing configurations in the $xy$-coordinate system is more intuitive but descriptor calculations are easier in $a_1a_2$-coordinate systems. Let $\vec{x}$ and $\vec{a}$ respectively be the coordinates of a point in the $xy$-coordinate system and $a_1a_2$-coordinate system. A given configuration $\config{p}$ can then be represented by different coordinates. Let $\mat{T}$ be the forward coordinate transformation matrix between $xy$- and $a_1a_2$-coordinate systems such that $\vec{a} = \mat{T} \vec{x}$ and $\vec{x} = \mat{T}^{-1} \vec{a}$. } 
	
	\begin{figure}
		\centering
		\includegraphics[width=0.6\columnwidth]{app2.eps}
	...
	Let $\mathcal{L}$ be the set of symmetries of the tiling of the plane, and $\mat{L}$ be one of the corresponding matrices written using the $xy$-coordinate system. Let $\vec{x}$ be the position of a disk and $\vec{x}' = \mat{L} \vec{x}$ the position of the disk under the action of $L \in \mathcal{L}$. \red{As an example, consider the yellow square in Fig.\ \ref{fig:app2} that is obtained by rotating the green square by $2\pi/3$ counter clockwise. The corresponding rotation matrix $\mat{L}$ represents this symmetry operation using the $xy$ representations of $\config{p}$ and $\config{p}^{\prime}$. The same operation can also be represented using the $a_1a_2$ representations of $\config{p}$ and $\config{p}^{\prime}$ as  }
	\begin{equation*} 
	\vec{a}' = \mat{T} \vec{x}' = \mat{T} \mat{L} \vec{x} = \mat{T} \mat{L} \mat{T}^{-1} \vec{a} = \mat{U} \vec{a}  
	\end{equation*}
	where $\mat{U} = \mat{T} \mat{L} \mat{T}^{-1}$. $\mat{U}$ is equivalent to $\mat{L}$, but is written using the $a_1a_2$-coordinate system. \red{This makes the descriptors $z_{\vec{k}}$ of two symmetric configurations be related to each other with the relation is precisely defined by $\mat{U}$.}
